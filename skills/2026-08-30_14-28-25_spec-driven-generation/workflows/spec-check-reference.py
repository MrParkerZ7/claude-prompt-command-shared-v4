...
Implements the seven checkable invariants in `specs/SPEC_DRIVEN_GENERATION.md`
...
# (7) value lineage — a 'documented' claim must name a source that resolves
led = os.path.join(ROOT, "0-script", "_data", "_lineage.json")
if os.path.isfile(led):
    try:
        LD = json.load(open(led, encoding="utf-8"))
        nbad = ndoc = 0
        for sh, blk in LD.get("sheets", {}).items():
            for cell, e in blk.get("cells", {}).items():
                if e.get("confidence") != "documented":
                    continue
                ndoc += 1
                src = (e.get("source") or "").split(" : ")[0].split(" · ")[0].strip()
                if not src or "/" not in src:
                    continue                       # prose, not a path claim
                hit = any(os.path.exists(os.path.join(b, src.replace("/", os.sep)))
                          for b in (os.path.dirname(ROOT), ROOT))
                if not hit:
                    nbad += 1
                    if nbad <= 5:
                        D(7, "lineage: %s!%s claims 'documented' but its source does not resolve -> %s"
                          % (sh, cell, src))
        if nbad > 5:
            D(7, "lineage: ...and %d more unresolvable 'documented' claims" % (nbad - 5))
        if not nbad:
            OK("lineage: all %d 'documented' values name a source that resolves" % ndoc)
    except Exception as e:
        print("  (lineage check skipped: %s)" % str(e)[:60])
else:
    OK("lineage: no ledger yet (optional) - run a builder that records one")
